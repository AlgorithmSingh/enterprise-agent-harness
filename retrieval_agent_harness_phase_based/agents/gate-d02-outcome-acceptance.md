...
Make every acceptance criterion binary, externally observable, and retrieval-specific: the retrieved and cleaned data answers the request correctly; every record delivered to the inference step honors the minimal cleaned-record contract, with no raw backend payload passing through; rate-limit compliance is demonstrated in recorded-fixture replay — `Retry-After` respected as a strict minimum delay, the utilization ceiling enforced as a per-window dispatch allowance and never exceeded, no unhandled 429, and budget-bucket isolation preserved so a drained bucket pauses only its own work; invalid provider inputs and continuations are refused before I/O; oversize or cancelled provider output is incomplete rather than truncated into success; healing stays within its per-script bound, uses only bounded redacted structural evidence, cannot edit outside the failing script or weaken checks, promotes a new immutable version only after unchanged offline validation passes, preserves the prior version on failure, and records every proposal and result in the healing ledger; and the deterministic test suite passes without network access. Ground each backend-facing criterion in the governing chapter under `docs/book/` — for GitHub, `docs/book/github-rate-limits.md` — rather than a remembered limit.
