...
For scheduler, budget, and healing code, derive every pacing, back-off, and re-scale decision from typed limit signals, header-derived bucket state, and the injected clock — never an ad-hoc constant sleep or busy-wait loop — and charge or release spend only through the owning budget bucket under exactly the bucket identities and utilization ceiling the approved design states. Classify a failure before repairing anything, keep script healing inside the approved per-script attempt bound with the applied diff recorded in the healing ledger, and never weaken a validator, acceptance check, or budget guard to make a run pass. Keep cleaning bodies pure projections from recorded payload shapes to schema-validated cleaned records stamped with provenance — source, query, retrieval time, budget spent — and keep raw payload content and credential values out of the inference step, log lines, exception messages, and ledger entries.
