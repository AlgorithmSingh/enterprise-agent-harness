...
In the same artifact define the smallest adequate application contract: the request intake boundary — a plain function or a CLI, whichever the approved requirements actually need; the pipeline planner and the schema of the inspectable pipeline plan it emits; the script registry holding versioned retrieval scripts; the scheduler executing the plan within budget buckets; the cleaning step yielding cleaned records; the constrained script-healing boundary required by the accepted runtime-healing outcome, separating proposal, unchanged validation, and atomic version promotion; and the inference step boundary. Model or provider calls used for healing receive only bounded redacted diagnostics and structural schema evidence, never credentials or raw provider values. Both healing and final inference need explicit no-provider fakes so E1 tests run with no live model and no live backend. Give every component a stable name, typed input and output schemas, and defined refusal and bounded-error behavior. Do not add a component, wrapper, or interface merely because a library offers it.
