...
Finalize supported callable names, sync or async form — the approved concurrency model for scheduled parallel retrieval decides which seams must be async, and a mixed form needs an approved boundary — parameter names and order, positional or keyword-only form, precise types, explicit named dependency seams, safe `None` sentinel defaults, return types, iterator or async semantics, modeled exception behavior including typed limit and transport failures, and annotation imports within approved architectural edges. Preserve B17's valid declarations, inheritance, fields, assignments, and structure.
