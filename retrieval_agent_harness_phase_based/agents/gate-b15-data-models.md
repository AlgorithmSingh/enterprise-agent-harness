...
Use frozen dataclasses, or the approved equivalent, for cleaned records, pipeline plans, retrieval-script identities and parameters, bounded redacted healing diagnostics, healing proposals and validation outcomes, and provenance- and healing-ledger entries; give every cleaned record its schema version, deterministic serialization, and the mandatory provenance fields — source, query, retrieval time, and budget spent — exactly as D06 defines them. Use mutable forms only where D06 approves mutation, such as budget-bucket state, with `default_factory` for mutable fields; enums or literals for closed sets such as backends, budget classes, failure classifications, and transport outcomes including cancelled and oversize; `TypedDict` only for dictionary-shaped boundaries; and Pydantic only where approved runtime parsing of external payloads requires it. Keep cleaned-record, bounded raw-payload boundary, pipeline-plan, bucket-state, cursor, dossier, healing proposal, validation outcome, and ledger shapes distinguishable, and never encode subprocess handles, HTTP clients, MCP sessions, credential values, or raw backend payloads in a persisted record or healer input. Keep validation pure and limited to approved invariants; add no business logic, I/O, orchestration, resource construction, or hidden globals.
