...
Every backend boundary validates typed inputs against an allowlisted host, operation, and parameter schema. Subprocesses use argument vectors, never shell strings; HTTP redirects may not escape the approved provider hosts; and gh, HTTP, and MCP calls have an explicit deadline plus configured byte or content-block limits for response bodies, stdout, stderr, and tool results. On cancellation or oversize output, stop dispatch, terminate and drain owned resources, record a classified incomplete outcome, and never truncate the data into apparent success or pass it to a cleaner or inference step.
...
Model rate-limit budgets as separate buckets exactly as the book defines them. GitHub tracks primary `core`, `search`, `code_search`, and `graphql` resources per credential, the endpoint-scoped 900-point REST minute ceiling and the 2,000-point GraphQL minute ceiling with client-side counters, and the documented CPU/concurrency signals; the 100-request in-flight cap spans REST and GraphQL and is a semaphore, not spent-budget accounting. Atlassian REST uses tenant/endpoint/method burst buckets plus either the default app-global hourly points pool across every tenant or an approved app-and-tenant pool; API-token traffic is outside that points quota. The Rovo MCP budget is per site per hour and observable only by a conservative client-side counter. Datadog buckets by `X-RateLimit-Name` learned from response headers. Never share one limiter across distinct buckets, split a genuinely shared pool, assume an undocumented budget, or let one drained bucket stall work funded by another.
...
On any limit signal, treat `Retry-After` as a strict minimum when present — never dispatch to that bucket earlier, and jitter re-entry so paused workers decorrelate; otherwise follow the book's per-backend protocol: GitHub sleeps to `x-ratelimit-reset` when remaining is zero and otherwise backs off at least 60 seconds exponentially; Atlassian REST dispatches on `RateLimit-Reason` (a global-pool quota pauses every tenant for the app, a tenant-pool quota pauses that tenant, and burst throttles only the offending endpoint); Rovo MCP pauses the site counter using its observed `Retry-After` when available; Datadog freezes the named bucket for `X-RateLimit-Reset` seconds. Retries use exponential backoff with full jitter, a bounded attempt count, and the retry-volume budget from the book. Cancellation or a pipeline deadline suppresses queued and future retries. Recovery keeps three numbers distinct — the multiplicative cut at the event, the recovery floor at resume, and the cut level as the slow-start ceiling — re-growing by doubling to that ceiling and additively beyond it, never a jump back to prior parallelism. Busy-wait retry against a limited backend is forbidden.

Classify a failure before repairing anything. A deterministic script or contract bug is fixed at the producer — the script or template — with the diff recorded in the healing ledger. A transient external failure (429, 5xx, network, timeout) gets bounded backoff-retry without editing any script. Upstream API drift heals the script and corrects the affected book chapter together. The healer receives only bounded, redacted diagnostics and structural schema evidence, may propose changes only to the failing registered script, and promotes an immutable new script version only after the unchanged compile, contract, recorded-fixture, and safety checks pass in a workspace where external reads are disabled; failure preserves the prior current version. Healing is bounded per script per run, audited, and never weakens a validator, acceptance check, cleaner, or budget guard to make a run look green.
