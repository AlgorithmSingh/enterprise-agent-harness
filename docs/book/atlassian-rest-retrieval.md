...
- **On 429, dispatch on `RateLimit-Reason`:** `*-quota-global-based` pauses every tenant using that app's Tier 1 global pool until the UTC-hour reset; `*-quota-tenant-based` pauses only that tenant's reviewed Tier 2 pool; `jira-burst-based` backs off that endpoint only; `jira-per-issue-on-write` delays writes to that one issue (limits: 20/2 s, 100/30 s) while other traffic continues.
...
|---|---|---|
| `429` + `RateLimit-Reason: jira-quota-global-based` / `jira-quota-tenant-based` / `confluence-quota-*` + `Retry-After` | Hourly points quota exhausted (app traffic); the global reason blocks every app tenant, while the tenant reason blocks that tenant | Sleep `max(Retry-After, time-to-next-UTC-hour)`; pause the exact global or tenant pool named by the reason; reduce per-request cost (`fields`, pagination) before resuming |
