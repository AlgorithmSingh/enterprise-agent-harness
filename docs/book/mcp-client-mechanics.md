...
1. **Protocol error** → JSON-RPC `error` object. The spec assigns `-32602` to invalid params, but the channel an unknown tool or bad argument actually takes is **server-dependent**: the reference Python server funnels every `tools/call` handler failure — unknown tool name, missing required argument, wrong-typed argument — into a successful response with `isError: true` (`lowlevel/server.py` wraps all handler exceptions), and extra or misspelled argument names are **silently ignored** with the tool running on defaults. Verified against `mcp` 1.29.0 in `.prototype/002-mcp-headless-client/`. A deterministic client therefore must (a) validate arguments client-side against the tool's `inputSchema` from `tools/list` before calling, and (b) classify failures by `isError` content, distinguishing "wrong tool/arguments — never retry unchanged" from "tool failed — maybe retry"; it cannot key retry policy on exception type.
...
| JSON-RPC `error` `-32602` | Invalid params / unknown tool / invalid cursor | Fix the call; do not retry unchanged |
