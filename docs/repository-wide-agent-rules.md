...
- Every backend boundary validates typed inputs against an allowlisted host, operation, and parameter schema; invokes subprocesses with argument vectors rather than a shell; and enforces a configured deadline plus bounded response, stdout, stderr, and MCP-content sizes. Oversize or cancelled work is terminated and classified, never truncated into apparent success or forwarded to inference.
...
- Classify failures before repair: deterministic script defects are fixed at the producer and recorded in the healing ledger; transient external failures get bounded backoff-retry without script edits; upstream API drift heals the script and the book chapter together. A healer receives only bounded, redacted diagnostics and structural schema evidence, may propose changes only to the failing registered script, and promotes an immutable new version only after the unchanged compile, contract, fixture, and safety checks pass in an external-read-disabled workspace. Healing is bounded per script per run and never weakens a validator or budget guard.
